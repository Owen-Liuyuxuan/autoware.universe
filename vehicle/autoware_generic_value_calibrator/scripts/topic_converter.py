...
class TopicConverter(Node):
    ...
    def __init__(self):
        ...
        self.get_logger().info(f'Topic Converter Started:')
    ...
    def _setup_conversions(self):
        """Setup available conversion functions"""
        if CONTROL_AVAILABLE:
            self.CONVERSION_MAP['control_acceleration'] = (
                Control, self._extract_control_acceleration
            )
            self.CONVERSION_MAP['control_velocity'] = (
                Control, self._extract_control_velocity
            )
            self.CONVERSION_MAP['control_steering_angle'] = (
                Control, self._extract_control_steering_angle
            )
            self.CONVERSION_MAP['control_steering_rate'] = (
                Control, self._extract_control_steering_rate
            )
        
        if ODOMETRY_AVAILABLE:
            self.CONVERSION_MAP['odom_linear_x'] = (
                Odometry, self._extract_odom_linear_x
            )
            self.CONVERSION_MAP['odom_linear_y'] = (
                Odometry, self._extract_odom_linear_y
            )
            self.CONVERSION_MAP['odom_angular_z'] = (
                Odometry, self._extract_odom_angular_z
            )
        
        if GEOMETRY_AVAILABLE:
            self.CONVERSION_MAP['twist_linear_x'] = (
                TwistStamped, self._extract_twist_linear_x
            )
            self.CONVERSION_MAP['twist_angular_z'] = (
                TwistStamped, self._extract_twist_angular_z
            )
            self.CONVERSION_MAP['accel_linear_x'] = (
                AccelStamped, self._extract_accel_linear_x
            )
