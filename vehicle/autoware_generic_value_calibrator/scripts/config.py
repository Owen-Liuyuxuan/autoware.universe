...
def load_indices_from_csv(csv_path):
    ...
    try:
        ...
        print(f"  Value range: [{value_list.min():.3f}, {value_list.max():.3f}] with {len(value_list)} points")
        print(f"  Velocity range: [{vel_list.min():.3f}, {vel_list.max():.3f}] m/s with {len(vel_list)} points")
