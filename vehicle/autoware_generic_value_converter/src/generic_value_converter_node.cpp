...
: Node("generic_value_converter", node_options)
{
  ...
  // ROS parameters
  const std::string csv_path_value_map =
    declare_parameter<std::string>("csv_path_value_map", "");
  max_value_ = declare_parameter<double>("max_value", 1.0);
  min_value_ = declare_parameter<double>("min_value", -1.0);
  ...
  if (convert_value_cmd_ && use_value_ff_) {
    ...
    if (!value_map_.getValue(desired_acc, current_velocity, output_value)) {
      RCLCPP_WARN_THROTTLE(
        ...
        "Failed to get value from map for acc=%.3f, vel=%.3f", desired_acc, current_velocity);
      output_value = 0.0;
    }
